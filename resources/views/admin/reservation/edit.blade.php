                ...
                <form method="post" action="{{ route('admin.reservation.update', $reservation->id) }}">
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-1">
                        ...
                        <div>
                            ...
                            <input type="text" id="no_telp" name="no_telp" value="{{ $reservation->no_telp }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="email">
                                @error('no_telp')
                                <div class="text-sm text-red-400">{{ $message }}</div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" id="reser_date" name="reser_date" value="{{ $reservation->reser_date }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="email">
                                @error('reser_date')
                                <div class="text-sm text-red-400">{{ $message }}</div>
                            ...
                            <div class="mt-1">
                                <select id="table_id" name="table_id" class="form-multiselect block w-full mt-1 rounded-lg">
                                    @foreach ($tables as $table)
                                        <option value="{{ $table->id }}" @selected($table->id == $reservation->table->id)>{{ $table->name }} ({{ $table->guest_number}} Guests)</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
